...
" 2>/dev/null
            ...
            # ============================================================================
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            echo "🔧 Slurm GRES (GPU) 설정 확인 및 수정..."
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

            # slurm.conf 경로 찾기
            SLURM_CONF=""
            for conf_path in /usr/local/slurm/etc/slurm.conf /etc/slurm/slurm.conf /opt/slurm/etc/slurm.conf; do
                if [ -f "$conf_path" ]; then
                    SLURM_CONF="$conf_path"
                    break
                fi
            done

            if [ -n "$SLURM_CONF" ]; then
                SLURM_ETC_DIR=$(dirname "$SLURM_CONF")
                GRES_CONF="$SLURM_ETC_DIR/gres.conf"
                NEED_RESTART=false

                # 1. GresTypes=gpu가 주석 처리되어 있으면 활성화
                if grep -q "^# *GresTypes=gpu" "$SLURM_CONF" 2>/dev/null; then
                    echo "  → GresTypes=gpu 활성화 중..."
                    sudo sed -i 's/^# *GresTypes=gpu/GresTypes=gpu/' "$SLURM_CONF"
                    NEED_RESTART=true
                elif ! grep -q "^GresTypes=" "$SLURM_CONF" 2>/dev/null; then
                    # GresTypes가 아예 없으면 추가
                    echo "  → GresTypes=gpu 추가 중..."
                    sudo sed -i '/^SlurmctldHost=/a GresTypes=gpu' "$SLURM_CONF"
                    NEED_RESTART=true
                else
                    echo "  ✅ GresTypes=gpu 이미 설정됨"
                fi

                # 2. gres.conf 파일 생성/업데이트 (항상 최신 상태 유지)
                echo "  → gres.conf 확인/생성 중..."
                # slurm.conf에서 Gres= 설정된 노드 찾기
                VIZ_NODES=$(grep "^NodeName=.*Gres=gpu" "$SLURM_CONF" | sed 's/NodeName=\([^ ]*\).*/\1/' | tr '\n' ' ' | sed 's/ $//')

                if [ -n "$VIZ_NODES" ]; then
                    # 각 노드별 GPU 타입 추출 및 gres.conf 생성
                    echo "  → GPU GRES 노드: $VIZ_NODES"

                    # gres.conf 헤더 생성
                    sudo bash -c "cat > '$GRES_CONF'" << 'GRESEOF'
# Slurm GRES Configuration
# Auto-generated by start.sh
# GPU 리소스 정의 (viz 노드용)
#
# AutoDetect=off: 실제 GPU 하드웨어 감지 비활성화
# File=: 가상 GPU 디바이스 파일 지정 (없어도 작동)
# 이 설정으로 실제 GPU가 없어도 GRES 리소스를 예약할 수 있음

AutoDetect=off
GRESEOF

                    # 각 노드에 대해 gres.conf 항목 추가
                    for node in $VIZ_NODES; do
                        # 해당 노드의 GPU 타입과 개수 추출
                        GRES_INFO=$(grep "^NodeName=$node " "$SLURM_CONF" | grep -oP 'Gres=gpu:\K[^:]+:\d+' || echo "generic:1")
                        GPU_TYPE=$(echo "$GRES_INFO" | cut -d: -f1)
                        GPU_COUNT=$(echo "$GRES_INFO" | cut -d: -f2)
                        [ -z "$GPU_COUNT" ] && GPU_COUNT=1

                        echo "NodeName=$node Name=gpu Type=$GPU_TYPE File=/dev/null Count=$GPU_COUNT" | sudo tee -a "$GRES_CONF" > /dev/null
                        echo "     $node: gpu:$GPU_TYPE:$GPU_COUNT"
                    done

                    sudo chmod 644 "$GRES_CONF"
                    sudo chown slurm:slurm "$GRES_CONF" 2>/dev/null || true
                    echo "  ✅ gres.conf 생성/업데이트 완료"
                    NEED_RESTART=true
...
" 2>/dev/null)
                    ...
                    echo "  ⚠️  Gres=gpu 설정된 노드 없음 - gres.conf 생성 건너뜀"
                fi

                # 3. Slurm 데몬 재시작 (설정이 변경된 경우)
                if [ "$NEED_RESTART" = true ]; then
                    echo "  → Slurm 설정 변경됨 - 데몬 재시작 중..."
                    sudo systemctl restart slurmctld 2>/dev/null || true
                    sleep 2
                    echo "  ✅ slurmctld 재시작 완료"
                fi
            else
                echo "  ⚠️  slurm.conf를 찾을 수 없음"
            fi
            echo ""
