...
class SlurmConfigManager:
    ...
    def restore_config(self, backup_path: str):
        ...
        try:
            subprocess.run(
                [SUDO, '-n', CP, '-p', backup_path, self.slurm_conf_path],
                check=True,
                capture_output=True
            )
    ...
    def apply_configuration(self, groups: List[Dict[str, Any]], 
        ...
        try:
            ...
            partitions_success = self.update_partitions(groups, skip_qos=False)
...
def apply_full_configuration(groups: List[Dict[str, Any]], 
                            dry_run: bool = False) -> Dict[str, Any]:
    """전체 설정 적용 (편의 함수)"""
    return slurm_config.apply_configuration(groups, dry_run)
