...
ws_disconnections_total = Counter('ws_disconnections_total', 'Total WebSocket disconnections')

# 현재 연결된 클라이언트 수
ws_clients_gauge = Gauge('ws_connected_clients', 'Number of connected WebSocket clients')

# 채널별 구독자 수
ws_channel_subscribers = Gauge('ws_channel_subscribers', 'Number of subscribers per channel', ['channel'])

# 메시지 카운터
ws_messages_sent = Counter('ws_messages_sent', 'Total messages sent', ['channel'])
ws_messages_received = Counter('ws_messages_received', 'Total messages received', ['type'])
...
async def websocket_handler(request):
    ...
    ws_clients_gauge.set(len(connected_clients))
    ...
    try:
        ...
        async for msg in ws:
            if msg.type == web.WSMsgType.TEXT:
                try:
                    ...
                    ws_messages_received.labels(type=data.get('type', 'unknown')).inc()
        ...
        ws_clients_gauge.set(len(connected_clients))
        for channel in subscriptions:
            ws_channel_subscribers.labels(channel=channel).set(len(subscriptions[channel]))
...
async def metrics_handler(request):
    ...
    # aiohttp는 content_type에 charset이 포함되면 안됨
    # CONTENT_TYPE_LATEST = 'text/plain; version=0.0.4; charset=utf-8'
    # 그래서 charset을 제거하고 별도로 설정
    return web.Response(
...
def create_app():
    ...
    app.router.add_get('/metrics', metrics_handler)  # Prometheus metrics 엔드포인트
