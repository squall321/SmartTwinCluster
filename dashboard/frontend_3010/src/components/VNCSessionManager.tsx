...
export const VNCSessionManager: React.FC = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.API_BASE_URL}/api/vnc/images`, {
      ...
      console.error('Error fetching VNC images:', error);
    }
  };
  ...
  const deleteSession = async (sessionId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.API_BASE_URL}/api/vnc/sessions/${sessionId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
              ...
              <div>
                ...
                {images.find(img => img.id === selectedImageId) && (
                  <p className="mt-1 text-sm text-gray-500">
                    {images.find(img => img.id === selectedImageId)?.description}
                  </p>
                )}
              </div>
