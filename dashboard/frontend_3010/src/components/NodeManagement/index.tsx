...
const NodeManagement: React.FC = () => {
  ...
  const fetchNodes = async () => {
    ...
    try {
      ...
      const url = `${API_CONFIG.API_BASE_URL}/api/nodes?_t=${timestamp}`;
  ...
  const fetchNodeDetail = async (nodeName: string) => {
    try {
      const response = await fetch(`${API_CONFIG.API_BASE_URL}/api/nodes/${nodeName}`);
  ...
  const executeNodeAction = async (action: 'drain' | 'resume' | 'reboot', nodeName: string) => {
    ...
    try {
      const response = await fetch(`${API_CONFIG.API_BASE_URL}/api/nodes/${action}`, {
        method: 'POST',
        headers: {
