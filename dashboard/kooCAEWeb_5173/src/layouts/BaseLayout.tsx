...
const BaseLayout = ({ children, isLoggedIn }: { children: ReactNode; isLoggedIn: boolean }) => {
  ...
  useEffect(() => {
    ...
    if (path.includes('abd-calculator')) setSelectedKey('abd-calculator');
    else if (path.includes('advanced-display-builder')) setSelectedKey('advanced-display-builder');
    else if (path.includes('elastic-to-rigid-builder')) setSelectedKey('elastic-to-rigid-builder');
    else if (path.includes('box-morph')) setSelectedKey('box-morph');
    else if (path.includes('warp-to-stress')) setSelectedKey('warp-to-stress');
    else if (path.includes('drop-attitude-generator')) setSelectedKey('drop-attitude-generator');
    else if (path.includes('full-angle-drops')) setSelectedKey('full-angle-drops');
    else if (path.includes('drop-weight-impact-test-generator')) setSelectedKey('drop-weight-impact-test-generator');
    else if (path.includes('login')) setSelectedKey('login');
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Header style={{ position: 'fixed', zIndex: 1, width: '100%', padding: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Menu
            ...
            subMenuCloseDelay={0.3}  // ★ 추가: 깜빡임 방지용
            style={{
              margin: 0,
              width: '100%',
      ...
      >
        ...
        <div style={{ width: '100%', backgroundColor: 'rgba(255,255,255,0.1)' }}>{children}</div>
      </Content>
    </Layout>
  );
};
