...
const TimeAnimatedStlViewerComponent = ({ stlUrl, positions, frame }) => {
    ...
    useEffect(() => {
        ...
        BABYLON.SceneLoader.ImportMeshAsync(null, '', stlUrl, scene, undefined, '.stl').then(result => {
    ...
    useEffect(() => {
        if (!meshRef.current || !positions.length || frame >= positions.length)
            return;
        ...
        const vb = new BABYLON.VertexBuffer(mesh.getEngine(), flatPositions, BABYLON.VertexBuffer.PositionKind, false);
        mesh.setVerticesBuffer(vb);
        mesh.refreshBoundingInfo();
    }, [frame, positions]);
    return (_jsx("div", { style: { width: '100%', height: '600px', position: 'relative' }, children: _jsx("canvas", { ref: canvasRef, style: { width: '100%', height: '100%' } }) }));
};
