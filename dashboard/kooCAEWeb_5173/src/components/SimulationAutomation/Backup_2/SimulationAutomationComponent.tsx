...
const SimulationAutomationComponent: React.FC = () => {
        ...
        <Select
          ...
          onChange={(v: AnalysisType) => {
            setScenarios((prev) => prev.map((r) => (r.id === row.id ? {
              ...
              analysisType: v,
              params:
                v === "fullAngleMBD" ? { mbdCount: 1000, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
  ...
  const handleBulkSetType = () => {
    ...
    setScenarios((prev) => prev.map((r) => selectedRowKeys.includes(r.id) ? ({
      ...
      params:
        bulkType === "fullAngleMBD" ? { mbdCount: 1000, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
  ...
  return (
    <Flex vertical gap={16} className="p-4">
      ...
      <Text type="secondary">TypeScript + Vite + React + Ant Design · OBJ/K 드래그&드롭 · 상단정렬 UI · 높이/표면 공통 옵션 · 누적 낙하 DOE×회수 매트릭스</Text>
