        ...
        onChange: () => {
            ...
            if (sel) {
                ...
                const newRows = kFileBufferRef.current.map((f) => ({
                    ...
                    params: { faTotal: 100, includeFace6: true, includeEdge12: true, includeCorner8: true, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" },
                }));
                setScenarios(prev => [...prev, ...newRows]);
            ...
            render: (_, row) => (_jsx(Select, { style: { width: "100%" }, size: "small", value: row.analysisType, onChange: (v) => {
                    setScenarios((prev) => prev.map((r) => (r.id === row.id ? {
                        ...
                        params: v === "fullAngleMBD" ? { mbdCount: 1000, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
                            v === "fullAngle" ? { faTotal: 100, includeFace6: true, includeEdge12: true, includeCorner8: true, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
    ...
    const handleBulkSetType = () => {
        ...
        setScenarios((prev) => prev.map((r) => selectedRowKeys.includes(r.id) ? ({
            ...
            params: bulkType === "fullAngleMBD" ? { mbdCount: 1000, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
                bulkType === "fullAngle" ? { faTotal: 100, includeFace6: true, includeEdge12: true, includeCorner8: true, angleSource: "lhs", heightMode: "const", heightConst: 1.0, heightMin: 0.5, heightMax: 1.5, surface: "steelPlate" } :
