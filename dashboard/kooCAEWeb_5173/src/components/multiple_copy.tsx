...
const MultipleStlViewerComponent: React.FC<MultipleStlViewerProps> = ({ models }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const cameraRef = useRef<BABYLON.ArcRotateCamera | null>(null);
  const [loading, setLoading] = useState(true);
  const [meshBounds, setMeshBounds] = useState<Record<string, { min: BABYLON.Vector3; max: BABYLON.Vector3 }>>({});
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '650px' }}>
      ...
      <canvas ref={canvasRef} style={{ width: '100%', height: '100%', display: 'block' }} />
        ...
        {/* 모든 stl 파일 이름과 위치 + 각 위치의 bounding box 출력 */}
