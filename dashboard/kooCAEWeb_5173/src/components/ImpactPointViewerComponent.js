...
const ImpactorPointViewerComponent = ({ url, impactPoints }) => {
    const canvasRef = useRef(null);
    const engineRef = useRef(null);
    const sceneRef = useRef(null);
    const centerRef = useRef(null);
    const pointsRef = useRef([]);
    const maxExtentRef = useRef(100); // Z 위치 조정을 위해 저장
    useEffect(() => {
    ...
    return (_jsx("div", { style: { position: 'relative', width: '100%', height: '600px' }, children: _jsx("canvas", { ref: canvasRef, style: { width: '100%', height: '100%', display: 'block' } }) }));
};
