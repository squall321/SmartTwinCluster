...
type FaceCell = Face | null;
type Rect = { x: number; y: number; w: number; h: number };

type NetLayout = {
  atlasW: number; // pixels (native drawing space)
  atlasH: number; // pixels (native drawing space)
  grid: FaceCell[][]; // arbitrary rows/cols with nulls
  cellRects: Record<string, Rect>; // key: c{c}r{r}
...
function faceSizeMM(face: Face, xMM: number, yMM: number, zMM: number) {
  switch (face) {
    case "+X":
    case "-X":
...
// 전개도 프리셋
type NetPreset = "Compact 3×2" | "Vertical Cross 3×4" | "Horizontal Cross 4×3" | "Strip 6×1";
const PRESETS: Record<NetPreset, FaceCell[][]> = {
  // 기존 3×2 (촘촘)
  "Compact 3×2": [
    ["+X", "-X", "+Y"],
    ["-Y", "+Z", "-Z"],
...
function buildLayout(
  ...
  // per-face pixel size
  const facePx: Record<Face, { w: number; h: number }> = {
    "+X": { w: 0, h: 0 }, "-X": { w: 0, h: 0 },
    "+Y": { w: 0, h: 0 }, "-Y": { w: 0, h: 0 },
    "+Z": { w: 0, h: 0 }, "-Z": { w: 0, h: 0 },
  };
  (Object.keys(facePx) as Face[]).forEach((f) => {
...
function drawHinges(ctx: CanvasRenderingContext2D, layout: NetLayout) {
...
function drawFaceFills(ctx: CanvasRenderingContext2D, layout: NetLayout, ppmm: number) {
  (Object.keys(layout.faceRects) as Face[]).forEach((f) => {
    ...
    if (pxPer20mm >= 40) {
      ...
      for (let x = r.x; x <= r.x + r.w; x += pxPer20mm) {
        ctx.beginPath(); ctx.moveTo(x + 0.5, r.y); ctx.lineTo(x + 0.5, r.y + r.h); ctx.stroke();
      }
...
function drawSeamsAndLabels(ctx: CanvasRenderingContext2D, layout: NetLayout, highlight?: Face | null) {
  (Object.keys(layout.faceRects) as Face[]).forEach((f) => {
    const r = layout.faceRects[f];
    ...
    const text = f;
    const pad = 4; const tw = ctx.measureText(text).width + pad * 2;
...
function drawCircleToImageData(img: ImageData, cx:number, cy:number, radius:number, paint:boolean) {
  ...
  for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
    ...
    if (dx*dx + dy*dy <= r2) {
      const idx = (y*W + x)*4;
      if (paint) { data[idx]=255; data[idx+1]=255; data[idx+2]=255; data[idx+3]=255; }
...
type PatternKind = "wrap-cross" | "band-h" | "band-v" | "border-only" | "corner-pads";
type FacePattern = { kind: PatternKind; params?: Record<string, number> } | null;
type FacePatternMap = Partial<Record<Face, FacePattern>>;

// 패턴을 patternCanvas 위에만 그림(흰색=테이프)
function drawPatternOnFace(ctx: CanvasRenderingContext2D, r: Rect, kind: PatternKind, params: Record<string, number>, ppmm: number) {
  ctx.save();
...
// ===== 3D Preview =====
type MiniCubePreviewProps = {
  xMM: number; yMM: number; zMM: number;
  layout: NetLayout;
  getMaskCanvas: () => HTMLCanvasElement | null;
  getPatternCanvas: () => HTMLCanvasElement | null;
  maskVersion: number; // version bump to rebuild texture
};

function MiniCubePreview({ xMM, yMM, zMM, layout, getMaskCanvas, getPatternCanvas, maskVersion }: MiniCubePreviewProps) {
  ...
  useEffect(()=>{
    ...
    function onMove(e: PointerEvent){
      if (!draggingRef.current || !cubeRef.current) return;
    ...
    const animate = ()=>{
      ...
      if (cubeRef.current && !draggingRef.current) cubeRef.current.rotation.y += 0.006;
    ...
    return ()=>{
      ...
      if (edgesRef.current) {
        ...
        (edgesRef.current.material as THREE.LineBasicMaterial).dispose();
      }
    };
...
export default function CuboidNetPainter() {
  ...
  const [patternKind, setPatternKind] = useState<PatternKind>("wrap-cross");
  const [selectedFaces, setSelectedFaces] = useState<Face[]>(["+X"]); // pattern 적용 대상
  ...
  // Wrap helper sets
  const WRAP_Y: Face[] = ["+X","-X","+Z","-Z"]; // Y축 둘레(옆면 띠)
  const WRAP_X: Face[] = ["+Z","-Z","+Y","-Y"]; // X축 둘레
  const WRAP_Z: Face[] = ["+X","-X","+Y","-Y"]; // Z축 둘레
  ...
  const maskCanvasRef = useRef<HTMLCanvasElement | null>(null); // freehand brush
  const patternCanvasRef = useRef<HTMLCanvasElement | null>(null); // generated patterns
  ...
  function redraw() {
    ...
    ctx.fillText(
      `Atlas: ${layout.atlasW}×${layout.atlasH}px  |  Scale: ${ppmm} px/mm  |  View: ${zoom === "fit" ? "Fit" : `${Math.round(Number(zoom)*100)}%`}`,
      8 * dpr, c.height - 8 * dpr
    );
  }
  ...
  function handlePointerMove(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (mode === "pattern") return; // pattern 모드에서는 그리지 않음
    paintAt(e);
  }
  ...
  function addWrapAround(axis: "X" | "Y" | "Z") {
    ...
    const spec: FacePattern = { kind: patternKind, params: { lineMM, bandMM, borderMM, padMM, gapMM } };
    setPatterns(prev => {
      const next = { ...prev };
  ...
  // Clear mask
  function clearAll(val: number = 0) {
    const m = maskCanvasRef.current; const img = imageDataRef.current; if (!m || !img) return;
    const d = img.data;
    if (val === 0) {
  ...
  function setZoomPct(p: number) { setZoom(Math.max(0.1, p / 100)); redraw(); }
    ...
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <Card size="small" style={{ borderRadius: 12 }}>
        <Row gutter={[12, 8]} align="middle">
          <Col xs={12} sm={8} md={6} lg={4}>
            ...
            <InputNumber value={xMM} min={1} style={{ width: "100%" }} onChange={(v)=>setXMM(Number(v))} />
          </Col>
          <Col xs={12} sm={8} md={6} lg={4}>
            <Text type="secondary">Y (mm)</Text>
            <InputNumber value={yMM} min={1} style={{ width: "100%" }} onChange={(v)=>setYMM(Number(v))} />
          </Col>
          <Col xs={12} sm={8} md={6} lg={4}>
            <Text type="secondary">Z (mm)</Text>
            <InputNumber value={zMM} min={1} style={{ width: "100%" }} onChange={(v)=>setZMM(Number(v))} />
          </Col>
          <Col xs={12} sm={8} md={6} lg={4}>
            <Text type="secondary">Scale (px/mm)</Text>
            <InputNumber value={ppmm} min={1} style={{ width: "100%" }} onChange={(v)=>setPPMM(Number(v))} />
          </Col>

          <Col xs={24} sm={12} md={8} lg={6}>
            <Text type="secondary">Net preset</Text>
            <Select
              value={preset}
              style={{ width: "100%" }}
              onChange={(v)=>setPreset(v as NetPreset)}
              options={(["Compact 3×2","Vertical Cross 3×4","Horizontal Cross 4×3","Strip 6×1"] as NetPreset[])
          ...
          <Col xs={24} md={12} lg={10}>
            ...
            <Space wrap>
              ...
              <Button onClick={()=>clearAll(0)} disabled={mode==="pattern"}>Clear</Button>
              <Button onClick={()=>clearAll(255)} disabled={mode==="pattern"}>Fill</Button>
            </Space>
          </Col>
        </Row>
      </Card>
      ...
      {mode === "pattern" && (
        <Card size="small" style={{ borderRadius: 12 }}>
          <Row gutter={[12, 8]} align="middle">
            <Col xs={24} md={8} lg={6}>
              <Text type="secondary">Faces</Text>
              <Select
                mode="multiple"
                value={selectedFaces}
                style={{ width: "100%" }}
                onChange={(vals)=>setSelectedFaces(vals as Face[])}
                options={faceOptions}
                placeholder="Select faces to apply"
              />
            </Col>
            <Col xs={24} md={8} lg={6}>
              ...
              <Select
                value={patternKind}
                style={{ width: "100%" }}
                onChange={(v)=>setPatternKind(v as PatternKind)}
                options={patternOptions as any}
              />
            </Col>

            {/* 공통 파라미터들 (패턴에 따라 사용되는 것만 적용됨) */}
            <Col xs={12} sm={8} md={6} lg={4}>
              <Text type="secondary">Line (mm)</Text>
              <InputNumber min={0.5} value={lineMM} onChange={(v)=>setLineMM(Number(v))} style={{ width:"100%" }}/>
            </Col>
            <Col xs={12} sm={8} md={6} lg={4}>
              <Text type="secondary">Band (mm)</Text>
              <InputNumber min={1} value={bandMM} onChange={(v)=>setBandMM(Number(v))} style={{ width:"100%" }}/>
            </Col>
            <Col xs={12} sm={8} md={6} lg={4}>
              <Text type="secondary">Border (mm)</Text>
              <InputNumber min={0.5} value={borderMM} onChange={(v)=>setBorderMM(Number(v))} style={{ width:"100%" }}/>
            </Col>
            <Col xs={12} sm={8} md={6} lg={4}>
              <Text type="secondary">Pad (mm)</Text>
              <InputNumber min={1} value={padMM} onChange={(v)=>setPadMM(Number(v))} style={{ width:"100%" }}/>
            </Col>
            <Col xs={12} sm={8} md={6} lg={4}>
              <Text type="secondary">Gap (mm)</Text>
              <InputNumber min={0} value={gapMM} onChange={(v)=>setGapMM(Number(v))} style={{ width:"100%" }}/>
            </Col>
