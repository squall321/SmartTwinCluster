...
def reboot_node():
    ...
    🔧 FIX: 에러 핸들링 개선 및 RebootProgram 사용
    """
    try:
        data = request.get_json()
        ...
        if MOCK_MODE:
            ...
            # 🔧 FIX: scontrol reboot 명령어 사용 (RebootProgram 자동 실행)
            logger.info(f"Production mode: Rebooting node {node_name} using scontrol")
            
            try:
                # scontrol reboot 명령어 실행
                # 이 명령어는 slurm.conf의 RebootProgram을 자동으로 실행합니다
                success, stdout, stderr = run_slurm_command(
