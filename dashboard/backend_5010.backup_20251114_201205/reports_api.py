...
def usage_report():
    ...
    # Production 모드: 실제 Slurm 데이터 수집
    if not MOCK_MODE and SLURM_COLLECTOR_AVAILABLE:
        print(f"📊 [PRODUCTION] Usage report: {start_date.date()} to {end_date.date()}")
        ...
        try:
            ...
            print(f"❌ Error collecting Slurm data: {e}")
            import traceback
            traceback.print_exc()
...
def download_report(report_type, format_type):
    ...
    if MOCK_MODE:
        print(f"💾 [MOCK] Download {report_type} report as {format_type} (period: {period})")
        ...
        try:
            ...
            if report_type == 'usage':
                # period 파라미터를 전달하기 위해 Flask test context 사용
                with current_app.test_request_context(f'?period={period}'):
...
def generate_report():
    ...
    if MOCK_MODE:
        print(f"📊 [MOCK] Generate {report_type} report as {format_type}")
...
def trends_analysis():
    ...
    if MOCK_MODE:
        print(f"📊 [MOCK] Trends analysis: {resource} for {period}")
