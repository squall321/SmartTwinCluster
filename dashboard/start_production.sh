...
PYWSRFB
                ...
                # 컨테이너 내부 TigerVNC 버전 (22.04 sif=1.12 / 24.04 sif=1.13 — 블랙리스트 동작 차이 확인용)
                echo "    ── 컨테이너 TigerVNC 버전 (블랙리스트 동작이 버전마다 다름) ──"
                $_vssh "$_vuser@$_vnode" "inst=\$(apptainer instance list 2>/dev/null | awk 'NR>1{print \$1; exit}'); [ -n \"\$inst\" ] && apptainer exec instance://\$inst sh -c 'dpkg -s tigervnc-standalone-server 2>/dev/null | grep ^Version || Xtigervnc -version 2>&1 | head -1' || echo '(instance 없음)'" 2>/dev/null | tr -d '\r' | sed 's/^/      /'
