    ...
    for service_info in "${services[@]}"; do
        ...
        if [[ "$DRY_RUN" == false ]]; then
            ...
            if [[ "$service_type" == "frontend" ]]; then
                ...
                http_code=$(curl -s -o /dev/null -w "%{http_code}" -m 3 "http://localhost:$port$path" 2>&1)
            ...
            if [[ "$is_healthy" == false ]]; then
                ...
                if systemctl is-active --quiet "$service_name.service" 2>/dev/null; then
                    is_healthy=true
                    log_warning "$service_name is starting (systemd running, HTTP not ready yet)"
                else
                    log_error "$service_name is not responding (port $port)"
                    all_healthy=false
                fi
            fi
        else
