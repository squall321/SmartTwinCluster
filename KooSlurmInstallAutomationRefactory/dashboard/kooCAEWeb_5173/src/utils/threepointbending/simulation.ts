import {
  ...
  export function runSimulation(
    ...
    analysis: AnalysisConditions
  ): SimulationResult {
    const loads: number[] = [];
    const deflections: number[] = [];
    const curvatures: number[] = [];
    const moments: number[] = [];
    const shapes: DeformedShape[] = [];
    ...
    for (
      ...
      const kappaArray = computeCurvatureDistribution(
        curvature,
        L,
        100,
        analysis.loadType,
        analysis.loadPosition,
        analysis.supportCondition
      );
  
      const { wArray } = integrateDeformation(
        kappaArray,
        L / 99
      );
  
      const shape: DeformedShape = {
