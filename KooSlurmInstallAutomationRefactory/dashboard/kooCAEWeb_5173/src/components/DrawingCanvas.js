...
const DrawingCanvas = forwardRef(({ glbUrl, glbFile, onModelBounds }, ref) => {
    ...
    const [hoverDomPos, setHoverDomPos] = useState(null);
    const [modelToStageScale, setModelToStageScale] = useState(null);
    ...
    const handleClick = (e) => {
        ...
        if (!pointer)
            return;
        const snapped = snapToGrid((pointer.x - stagePosition.x) / stageScale, (pointer.y - stagePosition.y) / stageScale);
        if (mode === 'poly') {
    ...
    const handleDragVertex = (shapeId, index, pos) => {
    ...
    const handleSelectShape = (shapeId, e) => {
        ...
        const domX = containerRect.left +
            pointerPos.x * stageScale +
            window.scrollX;
        const domY = containerRect.top +
    ...
    useImperativeHandle(ref, () => ({
        ...
        setShapes: (newShapes) => setShapes(newShapes), // ✅ 추가
    }));
    return (_jsxs("div", { style: {
            ...
            minHeight: '100vh',
            borderRadius: '24px',
        }, children: [_jsxs("div", { style: { marginBottom: 10 }, children: [_jsx("button", { style: {
                            ...
                            cursor: 'pointer',
                            marginRight: '10px',
                        }, onClick: () => setMode('poly'), children: "Poly" }), _jsx("button", { style: {
                            ...
                            cursor: 'pointer',
                            marginRight: '10px',
                        }, onClick: () => setMode('box'), children: "Box" }), mode === 'poly' && (_jsx("button", { onClick: handleFinishPoly, style: { marginLeft: 10 }, children: "Finish Poly" })), _jsxs("span", { style: { marginLeft: 20 }, children: ["Grid Size:", _jsx("input", { style: {
                                    backgroundColor: '#fff',
                                    color: '#000',
                                    border: '1px solid #007bff',
                                    borderRadius: '4px',
                                    ...
                                    width: 80,
                                    marginLeft: 5,
                                }, type: "number", value: gridSize, onChange: (e) => setGridSize(Number(e.target.value)) })] })] }), _jsx("div", { ref: wrapperRef, style: { width: "100%", height: "100vh" }, children: _jsxs(Stage, { width: stageSize.width, height: stageSize.height, ref: stageRef, scaleX: stageScale, scaleY: stageScale, x: stagePosition.x, y: stagePosition.y, onClick: handleClick, onWheel: handleWheel, onMouseDown: (e) => {
                        if (e.evt.button === 1) {
                            setIsPanning(true);
                        ...
                        document.body.style.overflow = ""; // ★ 추가
                    }, style: { border: '1px solid gray', cursor: isPanning ? 'grab' : 'default' }, children: [(glbUrl || glbFile) && (_jsx(GLBLayer, { glbUrl: glbUrl ?? undefined, glbFile: glbFile ?? undefined, voxelCount: 50, onBounds: handleModelBounds })), _jsxs(Layer, { children: [(() => {
                                    ...
                                    for (let x = startX; x * stageScale + stagePosition.x < stageSize.width; x += gridSize) {
                                        vLines.push(_jsx(Line, { points: [x, worldTop, x, worldBottom], stroke: "#ddd", strokeWidth: gridLineWidth }, `v-${x}`));
                                    }
                                    const hLines = [];
                                    const startY = Math.floor(worldTop / gridSize) * gridSize;
                                    const endY = Math.ceil(worldBottom / gridSize) * gridSize;
                                    for (let y = startY; y * stageScale + stagePosition.y < stageSize.height; y += gridSize) {
                                        hLines.push(_jsx(Line, { points: [worldLeft, y, worldRight, y], stroke: "#ddd", strokeWidth: gridLineWidth }, `h-${y}`));
                                    }
                                    return [...vLines, ...hLines];
                                })(), shapes.map((shape) => {
                                    if (shape.type === 'poly') {
                                        return (_jsx(Line, { points: shape.points.flatMap((p) => [p.x, p.y]), closed: true, fill: "rgba(255, 0, 0, 0.3)", stroke: "red", strokeWidth: shapeStrokeWidth, onClick: (e) => {
                                                e.cancelBubble = true;
                                                setEditingShapeId(shape.id);
                                            }, onContextMenu: (e) => handleSelectShape(shape.id, e) }, shape.id));
                                    }
                                    else if (shape.type === 'box') {
                                        return (_jsx(Rect, { x: shape.canvasRect.x, y: shape.canvasRect.y, width: shape.canvasRect.width, height: shape.canvasRect.height, fill: "rgba(255, 0, 0, 0.3)", stroke: "red", strokeWidth: shapeStrokeWidth, onClick: (e) => {
                                                e.cancelBubble = true;
                                                setEditingShapeId(shape.id);
                                            }, onContextMenu: (e) => handleSelectShape(shape.id, e) }, shape.id));
                                    }
                                    ...
                                    shapes
                                        ...
                                        .flatMap((shape) => {
                                        if (shape.type === 'poly') {
                                            return shape.points.map((p, i) => (_jsx(Circle, { x: p.x, y: p.y, radius: vertexRadius, fill: "blue", draggable: true, onDragMove: (e) => {
                                            ...
                                            const corners = getBoxCorners(shape.canvasRect);
                                            return corners.map((p, i) => (_jsx(Circle, { x: p.x, y: p.y, radius: vertexRadius, fill: "blue", draggable: true, onDragMove: (e) => {
                                                    handleDragVertex(shape.id, i, {
                                                        x: e.target.x(),
                                                        y: e.target.y(),
                                                    });
                                        ...
                                        return [];
                                    }), currentPoints.length > 0 && (_jsxs(_Fragment, { children: [mode === 'poly' && (_jsx(Line, { points: currentPoints.flatMap((p) => [p.x, p.y]), closed: false, stroke: "red", strokeWidth: shapeStrokeWidth })), mode === 'box' && currentPoints.length === 2 && (_jsx(Rect, { x: Math.min(currentPoints[0].x, currentPoints[1].x), y: Math.min(currentPoints[0].y, currentPoints[1].y), width: Math.abs(currentPoints[1].x - currentPoints[0].x), height: Math.abs(currentPoints[1].y - currentPoints[0].y), stroke: "red", dash: [4, 4], strokeWidth: shapeStrokeWidth, fill: "rgba(255,0,0,0.1)" })), currentPoints.map((p, i) => (_jsx(Circle, { x: p.x, y: p.y, radius: vertexRadius, fill: "blue", draggable: true, onDragMove: (e) => {
                                                const snapped = snapToGrid(e.target.x(), e.target.y());
                                                const newPts = [...currentPoints];
                                                newPts[i] = snapped;
                                                setCurrentPoints(newPts);
                                            } }, i)))] }))] })] }) }), contextMenu.visible && (_jsx("div", { style: {
                    position: 'absolute',
                    top: contextMenu.y,
                    left: contextMenu.x,
                    ...
                    padding: 5,
                    zIndex: 10,
                }, children: _jsx("button", { onClick: handleDeleteShape, children: "\uC0AD\uC81C" }) })), hoverPos && hoverDomPos && (_jsxs("div", { style: {
                    position: 'absolute',
                    top: hoverDomPos.y,
                    left: hoverDomPos.x,
                    backgroundColor: 'white',
                    ...
                    pointerEvents: 'none',
                    zIndex: 100,
                }, children: ["(", hoverPos.x.toFixed(3), ", ", hoverPos.y.toFixed(3), ")"] }))] }));
});
