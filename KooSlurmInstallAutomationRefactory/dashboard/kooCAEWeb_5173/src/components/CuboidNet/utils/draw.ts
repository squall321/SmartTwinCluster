...
export function drawHinges(ctx: CanvasRenderingContext2D, layout: NetLayout) {
  ...
  for (let r = 0; r < nRows; r++) for (let c = 0; c < nCols - 1; c++) {
  ...
  for (let r = 0; r < nRows - 1; r++) for (let c = 0; c < nCols; c++) {
...
export function drawFaceFills(ctx: CanvasRenderingContext2D, layout: NetLayout, ppmm: number) {
...
export function drawSeamsAndLabels(ctx: CanvasRenderingContext2D, layout: NetLayout, highlight?: Face | null) {
  (Object.keys(layout.faceRects) as Face[]).forEach((f) => {
    const r = layout.faceRects[f];
    ...
    const text = f; const pad = 4; const tw = ctx.measureText(text).width + pad * 2;
...
export function drawCircleToImageData(img: ImageData, cx:number, cy:number, radius:number, paint:boolean) {
