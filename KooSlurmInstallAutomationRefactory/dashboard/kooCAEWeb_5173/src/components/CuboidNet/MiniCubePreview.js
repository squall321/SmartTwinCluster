...
export default function MiniCubePreview({ xMM, yMM, zMM, layout, getMaskCanvas, getPatternCanvas, version }) {
    ...
    useEffect(() => {
        ...
        const animate = () => { rafRef.current = requestAnimationFrame(animate); if (cubeRef.current && !draggingRef.current)
            cubeRef.current.rotation.y += 0.006; renderer.render(scene, camera); };
        animate();
        return () => {
            if (rafRef.current)
