export function generateRigidOptionFile(filename, allParts, excludedParts) {
...
export function generateRigidOptionText(filename, allParts, excludedParts) {
    const modeId = 1;
...
export function generateDropAttitudeOptionFile(kFileName, tableData, options) {
...
export function generateDropWeightImpactTestOptionFile(kFileName, locationXList, locationYList, initialVelocityXList, initialVelocityYList, initialVelocityZList, heightList, tFinal, youngModulusDamper, poissonRatioDamper, density, youngModulus, densityDamper, poissonRatio, type, dimensionDamper, meshSize, youngModulusFront, densityFront, poissonRatioFront, youngModulusWall, densityWall, poissonRatioWall, cylinderDimensions, dimension, offsetDistance) {
    let content = '*Inputfile\n';
    content += `${kFileName || 'UNKNOWN.k'}\n`;
    content += '*Mode\n';
...
export function generateDropWeightImpactTestbyPartsOptionFile(kFileName, partsIDs, initialVelocityXList, initialVelocityYList, initialVelocityZList, locationModes, heightList, tFinal, dt, density, youngModulus, poissonRatio, type, meshSize, youngModulusFront, densityFront, poissonRatioFront, youngModulusWall, densityWall, poissonRatioWall, cylinderDimensions, dimension, offsetDistance) {
    let content = '*Inputfile\n';
    content += `${kFileName || 'UNKNOWN.k'}\n`;
    content += `*Mode\n`;
    content += `DROP_WEIGHT_IMPACT_TEST,1\n`;
...
export function generateDimensionalToleranceOptionFile(kFileName, partsIDs, directions, variableMatrix) {
    let content = '*Inputfile\n';
    content += `${kFileName || 'UNKNOWN.k'}\n`;
    content += '*Mode\n';
