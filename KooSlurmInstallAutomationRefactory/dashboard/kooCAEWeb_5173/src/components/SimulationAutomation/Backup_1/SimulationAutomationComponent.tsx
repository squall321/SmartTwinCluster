...
export type HeightSource = "const" | "lhs"; // 높이: 상수값 또는 LHS 포함
export type DropType = "steelPlate" | "concreteBrick" | "custom"; // 낙하 타입
...
export interface ScenarioRow {
  ...
  params?: {
    ...
    // 공통(높이 및 낙하 타입) - MBD, fullAngle, fullAngleCumulative에서 사용
    heightSource?: HeightSource; // 높이 소스 (기본 const)
    heightConstValue?: number;   // 상수 높이값 (m)
    heightLhsMin?: number;       // LHS 최소 높이 (m)
    heightLhsMax?: number;       // LHS 최대 높이 (m)
    dropType?: DropType;         // 낙하 타입 (기본 steelPlate)
    dropTypeCustom?: string;     // 커스텀 타입명
