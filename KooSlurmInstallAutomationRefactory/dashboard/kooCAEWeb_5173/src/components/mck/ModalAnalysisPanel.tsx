...
const ModalAnalysisPanel: React.FC<ModalAnalysisPanelProps> = ({
  system,
  modalResults,
  onRunModal,
}) => {
  const [numModes, setNumModes] = useState<number>(5);
  ...
  const dataSource =
    modalResults?.frequencies.map((f, idx) => ({
      key: idx,
      mode: idx + 1,
  ...
  return (
    ...
    >
      <Form layout="inline" style={{ marginBottom: 12 }}>
        <Form.Item label="Number of Modes">
          <InputNumber
            min={1}
            value={numModes}
            onChange={(v) => setNumModes(v || 1)}
            style={{ width: 80 }}
          />
        </Form.Item>
        <Button type="primary" onClick={() => onRunModal(numModes)}>
          Run
        </Button>
      </Form>
      ...
      <Table
        ...
        rowClassName={(record) =>
          selectedMode === record.key ? 'ant-table-row-selected' : ''
        }
      />

      {selectedMode !== null && modalResults && (
        <Stage width={500} height={150} style={{ marginTop: 12 }}>
          <Layer>
            {Array.from({ length: system.masses.length }).map((_, i) => {
              const phi = modalResults.modeShapes[selectedMode][i];
              const yOffset =
                A *
                phi *
                Math.sin(
                  2 * Math.PI * modalResults.frequencies[selectedMode] * time
                );
