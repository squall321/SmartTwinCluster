...
export function runMotorLevelSimulation(config: MotorLevelSimulationConfig): MotorLevelSimulationResults {
  const {
    r_P_global_0,
    r_Q_global,
    xvector,
  ...
  // Force
  const forceDirNorm = Math.sqrt(
    config.forceDirection[0] ** 2 +
    config.forceDirection[1] ** 2 +
    config.forceDirection[2] ** 2
  ) || 1;
  ...
  for (let i = 1; i < timeSteps; i++) {
    omega_principal[i] = omega_principal[i - 1].map(
      (v, j) => v + alpha_principal[i - 1][j] * dt
    );
  }
  ...
  const a_B_average = magnitudeGrid.map(frame =>
    frame.reduce((sum, row) =>
      sum + row.reduce((s, v) => s + v, 0), 0
    ) / (gridX * gridY)
  );

  const rmsValue = Math.sqrt(
    a_B_average.reduce((sum, val) => sum + val * val, 0) / a_B_average.length
  );
