...
const ObjViewerComponent = ({ url, mtlUrl, backgroundColor = new BABYLON.Color4(1, 1, 1, 1), orthographic = true, }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                setLoading(true);
                const result = await BABYLON.SceneLoader.ImportMeshAsync(null, '', url, scene, undefined, '.obj');
                if (disposed)
                    return;
                const meshes = result.meshes.filter(m => m instanceof BABYLON.Mesh);
                if (meshes.length === 0)
                    throw new Error('OBJ에서 Mesh를 찾을 수 없습니다.');
    ...
    return (_jsxs("div", { style: { position: 'relative', width: '100%', height: '600px' }, children: [loading && (_jsx("div", { style: {
                ...
                }, children: "Loading..." })), _jsx("canvas", { ref: canvasRef, style: { width: '100%', height: '100%', display: 'block', zIndex: 0, position: 'relative' } })] }));
};
