...
type Triangle = [BABYLON.Vector2, BABYLON.Vector2, BABYLON.Vector2];
      ...
      (meshes) => {
        ...
        meshes.forEach((mesh) => {
          ...
          for (let i = 0; i < indices.length; i += 3) {
            const p0 = new BABYLON.Vector2(
              positions[indices[i] * 3],
              positions[indices[i] * 3 + 1]
            );
            const p1 = new BABYLON.Vector2(
              positions[indices[i + 1] * 3],
              positions[indices[i + 1] * 3 + 1]
            );
            const p2 = new BABYLON.Vector2(
        ...
        const gridArray: boolean[][] = Array.from({ length: rows }, () =>
        ...
        for (const tri of triangles) {
          ...
          for (let row = minRow; row <= maxRow; row++) {
            ...
            for (let col = minCol; col <= maxCol; col++) {
              ...
              const cellCenterX =
                xmin + (col + 0.5) * cellSize;
              const cellCenterY =
                ymin + (row + 0.5) * cellSize;

              if (pointInTriangle(
                new BABYLON.Vector2(cellCenterX, cellCenterY),
                a, b, c
              )) {
