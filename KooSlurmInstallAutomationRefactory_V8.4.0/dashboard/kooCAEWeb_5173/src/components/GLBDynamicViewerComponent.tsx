...
interface GLBDynamicViewerComponentProps {
  file: File;
  autoScale?: boolean; // 🔹 추가 옵션
}

const GLBDynamicViewerComponent: React.FC<GLBDynamicViewerComponentProps> = ({
  file,
  autoScale = true, // 🔹 기본값은 true
}) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [loading, setLoading] = useState(true);
  const [morphTargets, setMorphTargets] = useState<BABYLON.MorphTarget[]>([]);
  const [morphWeights, setMorphWeights] = useState<number[]>([]);
  ...
  const setupSceneHelpers = (scene: BABYLON.Scene) => {
    ...
    const showAxis = (size: number) => {
      const makeAxis = (name: string, color: BABYLON.Color3, dir: BABYLON.Vector3) => {
        const axis = BABYLON.MeshBuilder.CreateLines(
          name,
          { points: [BABYLON.Vector3.Zero(), dir.scale(size)], colors: [color.toColor4(), color.toColor4()] },
          scene
        );
  ...
  const loadGLBModel = async (
    ...
    try {
      ...
      const renderMeshes = scene.meshes.filter(
        (m) => m.getTotalVertices() > 0 && !['ground', 'X', 'Y', 'Z'].includes(m.name)
      );
      ...
      // Morph Targets
      const foundTargets: BABYLON.MorphTarget[] = [];
  ...
  const handleSliderChange = (value: number, index: number) => {
    setMorphWeights((prev) => {
      const next = [...prev];
      next[index] = value;
      if (morphTargets[index]) morphTargets[index].influence = value;
      return next;
    });
  };
  ...
  return (
    <div style={{ width: '100%', height: '600px', position: 'relative' }}>
      ...
      {!loading && morphTargets.length > 0 && (
        ...
        >
          <h4 style={{ marginBottom: 8 }}>Morph Targets</h4>
          {morphTargets.map((target, i) => (
            <div key={target.name || i} style={{ marginBottom: 12 }}>
              ...
              <Slider
                ...
                max={1}
                step={0.01}
                value={morphWeights[i]}
                onChange={(v) => handleSliderChange(v, i)}
              />
            </div>
          ))}
        </div>
