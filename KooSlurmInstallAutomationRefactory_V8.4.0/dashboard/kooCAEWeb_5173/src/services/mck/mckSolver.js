...
function reduceMatrix(M_full, freeDOFIndices) {
    return math.subset(math.subset(M_full, math.index(freeDOFIndices, math.range(0, M_full.size()[1]))), math.index(math.range(0, freeDOFIndices.length), freeDOFIndices));
}
export function modalAnalysis(system, numModes) {
    ...
    for (const mass of system.masses) {
        const hasConnection = system.springs.some((s) => s.from === mass.id || s.to === mass.id) ||
            system.dampers.some((d) => d.from === mass.id || d.to === mass.id);
        if (!hasConnection) {
            throw new Error(`Floating mass detected: ${mass.id}. Please connect it to springs or dampers.`);
        }
    }
    let M_full = math.zeros(n, n);
    let K_full = math.zeros(n, n);
    let C_full = math.zeros(n, n);
    // Mass matrix
    ...
    for (const spring of system.springs) {
        ...
        const k = spring.k;
        if (i !== undefined)
            K_full.set([i, i], K_full.get([i, i]) + k);
        if (j !== undefined)
            K_full.set([j, j], K_full.get([j, j]) + k);
    ...
    for (const damper of system.dampers) {
        ...
        const c = damper.c;
        if (i !== undefined)
            C_full.set([i, i], C_full.get([i, i]) + c);
        if (j !== undefined)
            C_full.set([j, j], C_full.get([j, j]) + c);
        if (i !== undefined && j !== undefined) {
    ...
    const K = K_full;
    const C = C_full;
    const zero = math.zeros(n, n);
    const I = math.identity(n);
    try {
        const Minv = math.inv(M);
        const A_top = math.concat(zero, I, 1);
        const A_bottom = math.concat(math.multiply(-1, math.multiply(Minv, K)), math.multiply(-1, math.multiply(Minv, C)), 1);
        const A = math.concat(A_top, A_bottom, 0);
        const eig = math.eigs(A);
        const lambdas = eig.values;
        const freqs = [];
        const dampingRatios = [];
        lambdas.forEach((val) => {
        ...
        throw new Error("Modal analysis failed due to matrix singularity or disconnected masses.");
    }
}
export function harmonicResponse(system, freqRange, numPoints) {
    const freeDOFIds = system.masses.map((m) => m.id);
    const dofIndexMap = {};
    freeDOFIds.forEach((id, i) => {
        dofIndexMap[id] = i;
    ...
    system.dampers.forEach((damper) => {
        ...
        const c = damper.c;
        if (i !== undefined)
            C.set([i, i], C.get([i, i]) + c);
        if (j !== undefined)
            C.set([j, j], C.get([j, j]) + c);
        if (i !== undefined && j !== undefined) {
    ...
    if (system.forces.length === 0) {
        ...
        system.forces.forEach((force) => {
            const F = math.zeros(n, 1);
            const index = dofIndexMap[force.targetMassId];
            if (index === undefined) {
                console.warn(`Force target ${force.targetMassId} is not a mass DOF, ignoring.`);
                return;
            }
            ...
            freqs.forEach((f) => {
                ...
                const Z = math.add(math.multiply(-omega * omega, M), math.add(math.multiply(math.complex(0, omega), C), K));
                const X = math.multiply(math.inv(Z), F);
                const Xval = X.get ? X.get([index, 0]) : X;
                let mag = math.abs(Xval);
                if (mag < 1e-20)
                    mag = 1e-20;
                const phase = Number(math.atan2(Number(math.im(Xval)), Number(math.re(Xval)))) *
                    (180 / Math.PI);
    ...
    return {
        frequency: freqs.map((f) => Number(f)),
        magnitudeDB,
        phaseDeg,
    };
}
export function runTransientAnalysis(system, forces) {
    ...
    const time = math.range(0, maxDuration + dt, dt, true).toArray();
    const Fhist = math.zeros(n, time.length);
    ...
    const Keff = math.add(K, math.add(math.multiply(a1, C), math.multiply(a0, M)));
    const KeffInv = math.inv(Keff);
    let x = math.zeros(n, 1);
    let v = math.zeros(n, 1);
    let a = math.multiply(Minv, math.subtract(math.column(Fhist, 0), math.add(math.multiply(C, v), math.multiply(K, x))));
    const displacements = [];
    for (let k = 0; k < time.length; k++) {
        const Fk = math.column(Fhist, k);
        const rhs = math.add(Fk, math.add(math.multiply(M, math.add(math.multiply(a0, x), math.add(math.multiply(a2, v), math.multiply(a3, a)))), math.multiply(C, math.add(math.multiply(a1, x), math.add(math.multiply(a4, v), math.multiply(a5, a))))));
        const x_new = math.multiply(KeffInv, rhs);
        const a_new = math.add(math.multiply(a0, math.subtract(x_new, x)), math.add(math.multiply(a2, v), math.multiply(a3, a)));
        const v_new = math.add(v, math.multiply(dt, math.add(math.multiply(1 - gamma, a), math.multiply(gamma, a_new))));
        displacements.push(x_new.toArray().map((val) => val[0]));
        x = x_new;
        v = v_new;
        a = a_new;
    }
...
function interpolateTable(tArr, fArr, time) {
    const fvec = [];
    for (const t of time) {
        if (t <= tArr[0]) {
            fvec.push(fArr[0]);
            ...
            for (let i = 0; i < tArr.length - 1; i++) {
                if (tArr[i] <= t && t <= tArr[i + 1]) {
                    const slope = (fArr[i + 1] - fArr[i]) / (tArr[i + 1] - tArr[i]);
...
export function transientResponse(system, time, forcesPerNode) {
    const freeDOFIds = system.masses.map((m) => m.id);
    const dofIndexMap = {};
    freeDOFIds.forEach((id, i) => {
        dofIndexMap[id] = i;
    ...
    for (let tIdx = 0; tIdx < time.length; tIdx++) {
        ...
        const beta = 0.25;
        const gamma = 0.5;
        const A = math.add(math.add(K, math.multiply(gamma / (beta * dt), C)), math.multiply(1 / (beta * dt * dt), M));
        const rhs = math.add(F, math.multiply(M, math.add(math.multiply(1 / (beta * dt * dt), X), math.multiply(1 / (beta * dt), V))), math.multiply(C, math.add(math.multiply(gamma / (beta * dt), X), math.multiply((gamma / beta - 1), V))));
        const Xnew = math.multiply(math.inv(A), rhs);
        const Vnew = math.add(math.multiply(gamma / (beta * dt), math.subtract(Xnew, X)), math.multiply(1 - gamma / beta, V));
        X = Xnew;
        V = Vnew;
    ...
    return {
        time,
        displacement: math.transpose(math.matrix(displacementHistory)).toArray(),
    };
}
