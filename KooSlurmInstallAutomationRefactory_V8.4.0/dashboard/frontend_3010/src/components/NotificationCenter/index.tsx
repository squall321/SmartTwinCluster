...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const response = await fetch('/dashboardapi/notifications');
      if (response.ok) {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const response = await fetch('/dashboardapi/notifications/mark-read', {
        method: 'POST',
      });
