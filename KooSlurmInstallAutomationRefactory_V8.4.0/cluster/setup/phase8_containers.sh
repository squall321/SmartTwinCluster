...
deploy_to_node() {
    ...
    for sif in $sif_files; do
        ...
        if timeout 30 ssh $SSH_OPTS ${user}@${ip} "sudo mv /tmp/${sif_name} $remote_path && \
            ...
            ssh $SSH_OPTS ${user}@${ip} "rm -f /tmp/${sif_name}" 2>/dev/null || true
            ((fail_count++))
        fi
    done
...
deploy_viz_images() {
    ...
    log_success "Viz-node image deployment complete"
    echo ""
}
...
deploy_compute_images() {
    ...
    log_success "Compute-node image deployment complete"
    echo ""
}
