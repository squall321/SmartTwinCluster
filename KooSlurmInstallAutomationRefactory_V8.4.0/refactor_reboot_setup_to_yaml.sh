#!/bin/bash
################################################################################
# setup_reboot_program.sh를 YAML 기반으로 리팩토링하는 스크립트
################################################################################
...
if [ -f "setup_reboot_program.sh" ]; then
    ...
    echo "⚠️  setup_reboot_program.sh 파일이 없습니다"
fi
echo ""
...
cat > setup_reboot_program.sh << 'EOF'
#!/bin/bash
################################################################################
...
# slurm.conf에 RebootProgram 추가 (YAML에서 이미 반영되었을 수 있음)
...
for node in $COMPUTE_NODES; do
    echo "   ssh-copy-id ${ADMIN_USER}@${node}"
done
echo ""
...
echo ""
echo "  3. setup_cluster_full.sh 통합 (선택사항):"
echo "     # SSH 설정 후 자동으로 실행되도록 수정"
echo ""
